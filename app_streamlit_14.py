...
if 'map_anclado_estado' not in st.session_state: st.session_state['map_anclado_estado'] = False

# ==========================================
# --- FUNCIONES AUXILIARES GEOMÉTRICAS ---
# ==========================================

def calcular_area_poligono(coords_geo):
    if coords_geo is None or len(coords_geo) < 3:
        return 0.0
    lat_media = np.mean(coords_geo[:, 1])
    lon_media = np.mean(coords_geo[:, 0])
    _, _, h_n, _ = utm.from_latlon(lat_media, lon_media)
    
    poly_m = np.array([utm.from_latlon(pt[1], pt[0], force_zone_number=h_n)[:2] for pt in coords_geo])
    return ShapelyPolygon(poly_m).area
...
vista_actual = st.sidebar.radio("📌 NAVEGACIÓN", ["🗺️ 1. Dibujo y Localización", "⚙️ 2. Configuración y Resultados"])
st.sidebar.divider()
...
if vista_actual == "🗺️ 1. Dibujo y Localización":
    ...
    output_mapa = st_folium(st.session_state['map_obj'], width="100%", height=600, key="visor_principal", returned_objects=["center", "zoom", "all_drawings"])
    ...
    if output_mapa and output_mapa.get("all_drawings") and len(output_mapa["all_drawings"]) > 0:
        ...
        if 'poligono_usuario' not in st.session_state or not np.array_equal(st.session_state['poligono_usuario'], np.array(nuevas_coords)):
            ...
            st.toast("✅ Polígono guardado. Ve a la Configuración en el menú lateral.")
            st.session_state['rebuild_map'] = True
            resetear_calculo() 
            st.rerun()
    ...
    with st.sidebar:
        ...
        def al_cambiar_capa():
            st.session_state['old_capa'] = st.session_state['ui_capa_base']
            st.session_state['rebuild_map'] = True
        ...
        if 'poligono_usuario' in st.session_state:
            area_m2_vivo = calcular_area_poligono(st.session_state['poligono_usuario'])
            st.divider()
            st.success(f"**📐 Área de la Parcela:**\n\n**{area_m2_vivo:,.2f} m²**\n\n**{area_m2_vivo/10000:,.4f} ha**")

# ------------------------------------------
# VISTA 2: CONFIGURACIÓN Y RESULTADOS
# ------------------------------------------
elif vista_actual == "⚙️ 2. Configuración y Resultados":
    if 'poligono_usuario' not in st.session_state:
        st.warning("⚠️ Ve a la sección 'Dibujo y Localización' en el menú lateral y dibuja un polígono primero.")
    else:
        with st.sidebar:
