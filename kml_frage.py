...
def parse_kmz(filename):
    ...
    for pm in root.findall(f'.//{prefix}Placemark', ns):
        ...
        if poly is not None: # Fixed DeprecationWarning
...
if polygons:
    ...
    plt.axis('equal')
    plt.savefig('mapa_numerado_v2.png')

    # Dataframe processing
    df = pd.DataFrame(all_data)
    
    # ROUNDING TO 2 DECIMALS
    df['UTM_Este'] = df['UTM_Este'].round(2)
    df['UTM_Norte'] = df['UTM_Norte'].round(2)
    
    df.to_csv('puntos_prospeccion_v2.csv', index=False)
    generate_simple_dxf('geometria_y_puntos_v2.dxf', coords, all_points_latlon, all_points_ids)
    
    print("Archivos generados: mapa_numerado_v2.png, puntos_prospeccion_v2.csv, geometria_y_puntos_v2.dxf")
    print(df.head())
else:
