...
def calcular_malla(coords_geo, dist, marg, metodo, off_x, off_y):
    ...
    # Protocolo de rescate (modificado a < 3 puntos)
    ...
    for lo, la in zip(p_lon_final, p_lat_final):
        ...
        # ID guardado directamente como texto para facilitar la edición, sin Lat/Lon
        data_pts.append({
...
with tab1:
    ...
    with col_map:
        if st.session_state['rebuild_map'] or st.session_state['map_obj'] is None:
            # MAPA ACTUALIZADO CON CONTROLES DE BLOQUEO
            m = folium.Map(
    ...
    with col_ctrl:
        ...
        st.divider()
        # TOGGLE DE ANCLAJE
        anclado = st.toggle("🔒 Anclar Mapa (Bloquea movimiento)", value=st.session_state['mapa_anclado'])
        if anclado != st.session_state['mapa_anclado']:
            st.session_state['mapa_anclado'] = anclado
            st.session_state['rebuild_map'] = True
            st.rerun()
...
with tab2:
    if 'poligono_usuario' not in st.session_state:
        ...
        with col_conf:
            ...
            paso = st.selectbox("Resolución botón:", [1.0, 0.5, 0.1, 0.01], format_func=lambda x: f"{x} m")
